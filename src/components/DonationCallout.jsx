    ...
    <section className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-indigo-600 via-violet-600 to-fuchsia-600 p-6 text-white shadow-lg">
      ...
      <div className="relative z-10 flex flex-col items-start gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          {['₹50','₹100','₹250'].map((tier) => (
            <button key={tier} className="rounded-full bg-white/90 px-4 py-2 font-medium text-slate-800 shadow hover:bg-white">
              {tier}
            </button>
          ))}
          <button className="rounded-full border-2 border-white/80 px-4 py-2 font-medium text-white backdrop-blur transition hover:bg-white/10">
            Custom
          </button>
        </div>
      </div>
